...
> **Note:** "Shipped" items below describe the MVP. Version 1 and 12-month items are targets. Compare against [current MVP](../reference/current-mvp.md) before treating a line as implemented.
...
### Shipped: epistemic database depth (2026-08-16)

- `claim_citations.evidence_span` populated from matching article section text
- claim `confidence` computed as the weakest supporting citation tier, with `confidence_metrics` persisted and returned on the claim
- contradiction detection on claim insert (`claim_conflict`, `source_conflict`), returned on the claim object
- incoming entity relationship traversal on `GET /api/v1/entities/{slug}` (outgoing first, then incoming)
- JSON-LD endpoints return `Content-Type: application/ld+json`
...
- evidence spans quote article section text, not ingested source documents
...
- every seeded claim has a non-empty `evidence_span` from article section text
- claim confidence scores are computed, not hand-set
- knowledge graph is traversable via outgoing and incoming entity relationship endpoints
- full knowledge graph is machine-exportable as JSON-LD with `application/ld+json`
...
- evidence spans from ingested source documents, not only article section text
- contradiction detection as a publication gate: flag conflicting claims before publication
...
- claims are extracted by pipeline, not only hand-seeded
- every claim has an `evidence_span` linking to source-document text
