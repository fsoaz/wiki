...
> **Note:** This document describes the **target** architecture. For what the running MVP actually does, see [current MVP](../reference/current-mvp.md). Features labeled "not in MVP" are design intent, not shipped behavior.
...
- backend: `FastAPI`
- database: `PostgreSQL` (MVP default is SQLite; Postgres is optional)
- search: `OpenSearch` (not in MVP; search is lexical over seeded article text)
- cache and queues: `Redis` (not in MVP)
- orchestration: `Docker`, `Kubernetes` (MVP ships Compose for Postgres only; no Kubernetes)
- AI: LLMs, embeddings, RAG, and workflow agents (not in MVP; article chat is grounded to seeded article text)

## Logical Architecture

This diagram is the target system. The MVP is a single FastAPI process plus Next.js, with SQLite or optional Postgres. There is no API gateway, event bus, or worker pool in this repository.
...
Shipped MVP fields (matches `apps/api` and `infra/sql/001_init.sql`):
...
Target fields not in the running schema:
...
Target design. The MVP search path is lexical matching over seeded article text. There is no OpenSearch cluster or vector store.
...
| `GET /api/v1/articles/{slug}/jsonld` | Article as Schema.org JSON-LD (`application/ld+json`) |
| `GET /api/v1/export?format=jsonld` | Paginated knowledge graph as JSON-LD `@graph` (`application/ld+json`) |

## Current MVP Interaction Layer

Shipped UI and API behavior lives in [current MVP](../reference/current-mvp.md). The live route list is in the [API reference](../reference/api.md).
...
Target controls. The MVP implements demo RBAC and an audit log for auth, submission, assignment, and review events. It does not implement the rest of this list.

For what the running code actually enforces, and the limitations it accepts, read [security](../reference/security.md). This section is the destination, not the current state.

- RBAC with least privilege and workflow-scoped permissions (MVP: seeded roles, Bearer sessions, and separation of duties on review decisions)
- MFA for experts and administrators (not in MVP)
- password or federated credentials (not in MVP: login is email-only)
- encryption in transit and at rest (not specified in the MVP; the MVP does hash session tokens at rest)
- secret management in vault-backed infrastructure (not in MVP)
- immutable audit trails for content and moderation events (MVP: append-only `audit_logs` rows)
- prompt and model version tracking for all AI-generated output (not in MVP)
- shared rate limiting, bot detection, and abuse heuristics on public APIs (the MVP limiter is process-local)
- source poisoning defense using reputation controls and anomaly scoring (not in MVP)

## Scalability and Reliability

These are design targets, not MVP SLOs. Redis, regional clusters, and Kubernetes are not in this repository.
